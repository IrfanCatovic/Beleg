...
import type { Role, TaskFormData, TaskForEdit } from './taskModalTypes'

const ROLE_OPTIONS: Role[] = ['admin', 'sekretar', 'vodic', 'blagajnik', 'menadzer-opreme']
...
export default function TaskFormModal(props: TaskFormModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isCreate) {
        ...
        await props.onSubmit(task.id, data)
      }
      if (isCreate) {
        setNaziv(emptyForm.naziv)
        setOpis(emptyForm.opis)
        setDeadline(emptyForm.deadline)
        setHitno(emptyForm.hitno)
        setAllowedRoles(emptyForm.allowedRoles)
        setAllowAll(emptyForm.allowAll)
      }
      props.onClose()
    } catch (err: unknown) {
      const fallback = isCreate ? t('taskModal.errors.create') : t('taskModal.errors.save')
  ...
  const headerIconClass = isCreate ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
  const namePlaceholder = isCreate ? t('taskModal.placeholders.taskName') : t('taskModal.placeholders.taskNameShort')
  const descriptionPlaceholder = isCreate
    ? t('taskModal.placeholders.description')
    : t('taskModal.placeholders.descriptionShort')
  const allRolesLabel = isCreate ? t('taskModal.fields.allRoles') : t('taskModal.fields.allShort')
      ...
      >
        <div className="px-5 sm:px-6 pt-5 sm:pt-6 pb-4 border-b border-gray-100 flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            <div className={`flex-shrink-0 h-10 w-10 rounded-xl flex items-center justify-center mt-0.5 ${headerIconClass.split(' ')[0]}`}>
              {isCreate ? (
                <svg className={`w-5 h-5 ${headerIconClass.split(' ')[1]}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold text-gray-900 tracking-tight">
                {isCreate ? t('taskModal.newTitle') : t('taskModal.editTitle')}
              </h2>
              <p className="text-[11px] sm:text-xs text-gray-500 mt-0.5">
                {isCreate ? t('taskModal.newSubtitle') : t('taskModal.editSubtitle')}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-5 sm:px-6 py-5 sm:py-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-medium text-gray-700">{t('taskModal.fields.deadline')}</label>
                <CalendarDropdown
                  value={deadline ?? ''}
                  onChange={(value) => setDeadline(value || null)}
                  placeholder={t('taskModal.placeholders.pickDate')}
                  minDate={isCreate ? dateToYMD(new Date()) : new Date().toISOString().slice(0, 10)}
              ...
              <div className="space-y-2.5 md:col-span-2">
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px] sm:text-xs">
                  ...
                  >
                    ...
                    />
                    <span className="text-xs font-semibold">{allRolesLabel}</span>
                  </label>
                </div>
                {isCreate && (
                  <p className="text-[10px] sm:text-[11px] text-gray-400">{t('taskModal.help.allRoles')}</p>
                )}
              </div>
                ...
                <div className={`rounded-xl border px-3.5 py-3 transition-all ${hitno ? 'border-rose-300 bg-rose-50' : 'border-gray-100 bg-gray-50/50'}`}>
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className={`text-[10px] mt-0.5 ${hitno ? 'text-rose-500' : 'text-gray-400'}`}>{t('taskModal.help.urgent')}</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setHitno((prev) => !prev)}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${hitno ? 'bg-rose-500' : 'bg-gray-300'}`}
                    >
                      <span
            ...
            <div
              className={`pt-3 border-t border-gray-100 ${
                isCreate ? 'flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3' : 'flex justify-end gap-2.5'
              }`}
            >
              {isCreate && (
                <p className="text-[10px] sm:text-[11px] text-gray-400">{t('taskModal.help.multiAssignees')}</p>
              )}
